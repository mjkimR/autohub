...
Options:
  ...
  -e, --email EMAIL            Email the operator signs in with (required the first time)
  -s, --app-secret PASSWORD    The operator's password (generated and printed once when omitted the first time)
...
Environment variables PROJECT_ID, REGION, OPERATOR_EMAIL, APP_SECRET, DATABASE_URL, DB_CONNECTION_NAME, SA_NAME can also be used.
EOF
  exit 0
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -e|--email) OPERATOR_EMAIL="$2"; shift 2 ;;
...
# 1. The operator's account. The hub creates it at startup from these two values and keeps its password equal to
# FIRST_USER_PASSWORD on every start (FIRST_USER_SYNC_PASSWORD), so this bundle stays the one place the password
# is changed. The database only holds an Argon2id hash of it.
OPERATOR_EMAIL_VAL="${OPERATOR_EMAIL:-$(existing_value FIRST_USER_EMAIL)}"
if [[ -z "$OPERATOR_EMAIL_VAL" ]]; then
  echo "Error: --email is required the first time (the address you sign in with)." >&2
  exit 1
fi
NEW_PASSWORD=""
if [[ -n "$RAW_APP_SECRET" ]]; then
  OPERATOR_PASSWORD_VAL="$RAW_APP_SECRET"
else
  OPERATOR_PASSWORD_VAL=$(existing_value FIRST_USER_PASSWORD)
  if [[ -z "$OPERATOR_PASSWORD_VAL" ]]; then
    NEW_PASSWORD=$(openssl rand -hex 16)
    OPERATOR_PASSWORD_VAL="$NEW_PASSWORD"
  fi
fi

# Signs the session tokens. Replacing it signs everyone out, so it is generated once and kept.
TOKEN_SIGNING_KEY_VAL=$(existing_value SECRET_KEY)
if [[ -z "$TOKEN_SIGNING_KEY_VAL" ]]; then
...
echo "    Sign in as: $OPERATOR_EMAIL_VAL"
if [[ -n "$NEW_PASSWORD" ]]; then
  echo "    Generated password (shown once; it also stays in the secret bundle):"
  echo "    $NEW_PASSWORD"
else
  echo "    Password unchanged unless --app-secret was given. A changed password takes effect when the service restarts:"
  echo "    'just update-password' changes it and restarts Cloud Run in one step."
fi
