...
async def test_webhook_follows_an_advanced_run_with_its_work_plan(monkeypatch, advanced):
    """A work item's merge releases dependents from the delivery; the lease holder does so when this one lost."""
    ...
    lifecycle.manual_advance = AsyncMock(side_effect=None if advanced else ProjectError(409, "lease"))

    webhook = GitHubWebhookUseCase(repo, runs, lifecycle)
    webhook._finish = AsyncMock()
    await webhook.process(
        "merged", {"repository": {"full_name": "owner/app"}, "pull_request": {"number": 7}}, event="pull_request"
    )
