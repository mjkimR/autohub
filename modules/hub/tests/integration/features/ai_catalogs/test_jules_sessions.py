...
from app.features.ai_catalogs.providers.jules import JULES_API_BASE_URL
...
class FakeJules:
    ...
    def handle(self, request: httpx.Request) -> httpx.Response:
        ...
        if (
            request.method == "GET"
            and request.url.path == "/v1alpha/sources"
            and ("GET", "/v1alpha/sources") not in self.responses
        ):
            return httpx.Response(
                200,
                json={"sources": [{"name": "sources/opaque-app-id", "githubRepo": {"owner": "owner", "repo": "app"}}]},
            )
...
async def test_an_admitted_session_is_created_counted_and_tracked(client, session, jules):
    ...
    [request] = [r for r in jules.requests if r.method == "POST"]
...
async def test_reaching_the_daily_limit_holds_the_catalog_without_creating_a_session(client, session, jules):
    ...
    assert not [r for r in jules.requests if r.method == "POST"]
...
@pytest.mark.parametrize("source_status", [200, 429])
async def test_source_preflight_failure_does_not_reserve_or_create_a_session(session, jules, source_status):
    catalog = await make_catalog(session)
    jules.responses[("GET", "/v1alpha/sources")] = (source_status, {"sources": []})
    with pytest.raises(ProjectError) as error:
        await make_service().start(
            JulesSessionPayload(
    ...
    assert error.value.status_code == (422 if source_status == 200 else 429)
    assert not any(request.method == "POST" for request in jules.requests)
    assert not list(await session.scalars(select(AICatalogSession)))
    assert not list(await session.scalars(select(AICatalogDispatch)))
    if source_status == 429:
        row = await session.get(AICatalog, catalog.id)
        assert row.availability_state == "quota_blocked"
