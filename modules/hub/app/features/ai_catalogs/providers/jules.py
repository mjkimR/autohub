...
class JulesApiError(ProviderRequestError):
    """Sanitized Jules failure: no response body, credentials, or request headers."""
...
class JulesClient:
        ...
        *,
        ...
        return await self._request(
            ...
            json={
                ...
                "title": title,
                "sourceContext": {
                    "source": source_name or await self.resolve_source(repository),
    ...
    async def resolve_source(self, repository: str) -> str:
        """Resolve the actual provider resource name for both production sessions and probes."""
        page_token = None
        for _ in range(20):
            ...
            for source in page.get("sources") or []:
                ...
                if f"{repo.get('owner')}/{repo.get('repo')}".lower() == repository.lower():
                    name = source.get("name")
                    if isinstance(name, str) and name.startswith("sources/"):
                        return name
                    raise JulesApiError("Jules source is missing its resource name", 422)
            page_token = page.get("nextPageToken")
            if not page_token:
                raise JulesApiError("Connect this repository in Jules before starting work", 422)
        raise JulesApiError("Jules source listing exceeded the page limit", 422)
